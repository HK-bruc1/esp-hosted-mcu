...
Host 框架用"合约"一词描述平台需要提供的能力集合。技术上，合约就是 C 结构体里的一组函数指针：

- 框架在头文件（如 `h_port_contract.h`）中声明能力需求。
- 每个能力对应结构体中的一个函数指针。
- 平台实现这些函数，然后把函数地址填入全局合约实例。
- 初始化时 `h_validate_contracts()` 检查必填 slot 是否非 NULL。
- 通过后，框架通过 wrapper（如 `h_malloc()`、`h_transmit()`）调用平台能力。

移植新平台时，只需要实现四个全局合约实例：

| 全局实例 | 所在文件 | 能力范围 |
|---|---|---|
| `g_h_osal` | `host/port/<port>/h_osal.c` | 内存、线程、互斥、队列、信号量、定时器、日志等 |
| `g_h_event` | `host/port/<port>/h_event.c` | 事件注册、注销、投递 |
| `g_h_transport` | `host/port/<port>/h_transport_<bus>.c` | transport 生命周期、收发、GPIO 等 |
| `g_h_wifi` | `host/port/<port>/h_wifi.c` | Wi-Fi 可移植类型与原生类型的双向转换 |

框架不关心平台具体如何实现，只关心：函数指针非 NULL、签名匹配、行为符合约定（如 timeout 单位、返回值语义）。
...
> 利用目标平台已有的 API，实现 Host 框架声明的能力，并把函数地址填入合约结构体。

平台可以是 FreeRTOS、Linux、JL-RTOS、Zephyr、RT-Thread 等；框架不关心底层是谁，只关心合约是否被履行。

### 2.6.2 如何确定框架需要哪些能力

| 方法 | 看什么 | 能得到什么 |
|---|---|---|
| 读合约头文件 | `host/port/include/h_port_contract.h` | 全部能力需求的函数签名和注释 |
| 读 wrapper 头 | `host/port/include/h_wrapper.h` | core 实际调用的入口名称 |
| 搜索调用点 | grep `h_xxx()` 在 `host/core/`、`host/api/` | 某个能力是否真的被使用、使用场景 |
| 看校验函数 | `host/core/src/h_init.c` 的 `h_validate_contracts()` | 哪些是 required，哪些是 optional |
| 看参考实现 | `host/port/linux/`、`host/port/esp-idf/` | 最小形状与完整语义的对比 |
| 编译驱动 | 先 stub 全部返回 NULL 或 H_OK | `h_validate_contracts()` 会明确提示缺什么 slot |

### 2.6.3 工程经验

1. **先编译，后运行**：先用 stub 让代码编过，再逐层填实现，不要一次性写完整。
2. **先最小，后完整**：第一阶段只开 UART，关闭 BT、OTA、network split、power-save。
3. **先 mock，后真实硬件**：Linux mock 验证 core 逻辑，再上板调时序。
4. **对照参考 port**：`linux` 看最小形状，`esp-idf` 看完整语义和边界处理。
5. **用断言保一致**：用 `_Static_assert` 检查 `h_wifi_*` 与 `ctrl_cmd_t` union 成员的大小/偏移。
6. **日志先行**：在 wrapper 和 contract 实现中加日志，确认调用路径和参数。
7. **分层打通**：OSAL → event → wifi contract → transport adapter → transport task → control serial → RPC。
8. **让运行时告诉你缺什么**：不要试图一次性读透所有代码，用 `h_validate_contracts()` 和实际运行驱动实现。

### 2.6.4 优秀跨平台框架的共同特点

| 特点 | 说明 |
|---|---|
| 清晰的抽象边界 | core 不 include 平台头，平台能力通过 narrow interface 注入。 |
| 能力拆分 | OSAL、event、transport、wifi 等合约分离，而不是一个大 vtable。 |
| fail-fast 校验 | 初始化时检查 contract 完整性，缺能力立即失败并提示。 |
| wrapper 层 | core 调用 `h_malloc()` 而不是直接访问 `g_h_osal.malloc`，便于替换和加 guard。 |
| 可选能力可置空 | 不需要的功能 slot 可以为 NULL，wrapper 安全处理。 |
| 参考实现 + mock | 至少一个完整生产 port 和一个最小 mock port。 |
| 配置与代码分离 | 平台选择、feature flags 由 `h_port_config.h` 控制，代码不硬编码。 |
| 文档与代码同构 | 移植步骤、目录结构、验证方法都对应到实际文件和函数。 |

Host 框架基本满足以上特点，因此移植路径是清晰的：看懂合约 → 用平台 API 实现 → 填入结构体 → 跑验证。
...
├── myrtos_app_init.c            # optional: 平台应用集成入口，调用 h_hosted_init()
└── tools/                       # optional
```

`myrtos_app_init.c` 是可选但强烈建议的文件。它的作用是在目标平台应用启动流程中调用 `h_hosted_init()`，例如通过 RTOS 的 initcall、module init 或 `main()` 调用。不要把这行调用散落在应用业务代码里，应该由 port 层统一收口。
...
实现 contract 和 lifecycle hook 后，还必须让目标应用在某个启动阶段调用 `h_hosted_init()`。推荐做法是把这行调用收口到 port 层，而不是散落在应用业务代码里。

### 10.5.1 ESP-IDF 风格（同步调用）

ESP-IDF 的 `app_main()` 本身就是一个 FreeRTOS task，因此 example 里直接同步调用：

```c
void app_main(void)
{
    // ...
    ESP_ERROR_CHECK(esp_hosted_init());  // 内部调用 h_hosted_init()
    // ...
}
```

阻塞在 `app_main()` 里不影响系统其他任务，因为 FreeRTOS 调度器已经运行。

### 10.5.2 单 app task RTOS 风格（异步 init task）

某些 RTOS（如 JL-RTOS）整个应用主循环跑在单个 `app_core` task 里。如果同步调用 `h_hosted_init()`，slave reset/handshake 期间的等待会阻塞开机和 UI。

这类平台推荐：**`late_initcall` / `module_initcall` 注册一个轻量函数，该函数只创建一个独立的 init task，由 init task 调用 `h_hosted_init()`**。
...
### 10.5.3 应用如何知道 Hosted 已 ready

框架会通过 `H_EVENT_HOSTED` 事件通知应用：

| 事件 ID | 含义 |
|---|---|
| `H_EVENT_HOSTED_CP_INIT` | slave 已初始化 |
| `H_EVENT_HOSTED_TRANSPORT_UP` | transport 链路已就绪 |
| `H_EVENT_HOSTED_TRANSPORT_DOWN` | transport 链路断开 |
| `H_EVENT_HOSTED_TRANSPORT_FAILURE` | transport 故障 |

应用注册 handler：

```c
static void on_hosted_event(void *event_data, size_t len, void *ctx)
{
    h_event_hosted_t *ev = (h_event_hosted_t *)event_data;
    if (ev->id == H_EVENT_HOSTED_TRANSPORT_UP) {
        /* 可以调用 Wi-Fi / RPC API 了 */
    }
}

h_event_register(H_EVENT_HOSTED, H_EVENT_HOSTED_TRANSPORT_UP,
...
| 平台特性 | 推荐集成方式 | 原因 |
|---|---|---|
| `app_main()` 本身是独立 task（如 ESP-IDF FreeRTOS） | 同步调用 `esp_hosted_init()` / `h_hosted_init()` | 阻塞只影响该 task，不影响系统 |
| 整个应用跑在单 `app_core` task（如 JL-RTOS） | `late_initcall` / `module_initcall` + 独立 init task | 避免阻塞开机和 UI 主循环 |
| 有明确的 module init 阶段 | 在 module init 中同步或异步调用 | 与平台模块初始化风格保持一致 |

核心原则：

1. **收口到 port 层**：不要让应用业务代码直接调用 `h_hosted_init()`。
2. **不要阻塞系统启动**：如果平台只有一个应用 task，用异步 init task。
3. **用事件通知 ready**：应用通过 `H_EVENT_HOSTED` 监听 `TRANSPORT_UP` 或 `CP_INIT`，而不是轮询或硬编码延时。
4. **能通过 feature flag 关闭**：从 Makefile / port.cmake 中移除 `<port>_app_init.c` 即可断开 Hosted，不改动应用代码。
...
8. 增加 `<port>_app_init.c`，在平台启动流程中调用 `h_hosted_init()`（同步或异步，视平台 task 模型而定）。
9. 打通 slave reset / bus ready / private handshake。
10. 打通 control serial read/write。
11. 跑通最小 RPC，例如获取 MAC、获取版本或 Wi-Fi init。
12. 跑通 STA scan / connect。
13. 再启用 BT、OTA、power-save、network split 等可选功能。
...
- `h_hosted_init()` 成功完成。
- 平台应用启动流程中成功调用了 `h_hosted_init()`（通过 `<port>_app_init.c` 收口，同步或异步视平台 task 模型而定）。
- 应用能通过 `H_EVENT_HOSTED` 事件感知 transport ready / slave init 状态。
