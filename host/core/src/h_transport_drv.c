...
static void transport_driver_event_handler(uint8_t event)
{
	switch(event)
	{
		case TRANSPORT_TX_ACTIVE:
		{
			...
			H_LOGI(TAG, "Base transport is set-up, TRANSPORT_TX_ACTIVE");
...
{
	H_LOGI(TAG, "set_transport_state: %u", state);
...
h_err_t transport_drv_reconfigure(void)
{
	...
	H_LOGI(TAG, "Attempt connection with slave: retry[%u]", retry_slave_connection);
...
h_err_t transport_drv_remove_channel(transport_channel_t *channel)
{
	...
	case ESP_STA_IF:
		...
		// REMOVED: reg_rxcb — handled by port transport vtable
		break;
	case ESP_SERIAL_IF:
		/* TODO */
		break;
...
static h_err_t transport_drv_sta_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_STA_IF]) {
		H_LOGE(TAG, "Transport TX not ready or STA channel is not available, drop pkt");
...
static h_err_t transport_drv_ap_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_AP_IF]) {
		H_LOGE(TAG, "Transport TX not ready or AP channel is not available, drop pkt");
...
h_err_t transport_drv_serial_tx(void *h, void *buffer, size_t len)
{
	...
	if (!is_transport_tx_ready() || !chan_arr[ESP_SERIAL_IF]) {
		...
		H_LOGE(TAG, "Transport TX not ready or serial channel is not available, drop pkt");
		...
		transport_channel_tx_fn_t *tx, const transport_channel_rx_fn_t rx)
{
	...
	if (chan_arr[if_type]) {
		H_LOGW(TAG, "Channel [%u] already created, replacing with new callbacks", if_type);
	...
	default:
		//*tx = transport_drv_tx;
		H_LOGW(TAG, "Not yet supported ESP_Hosted interface for if_type[%u]", if_type);
		return NULL;
	}
...
{
	H_LOGI(TAG, "capabilities: 0x%x",cap);
}
...
static uint32_t process_ext_capabilities(uint8_t * ptr)
{
	...
	cap = (uint32_t)ptr[0] +
		...
		((uint32_t)ptr[2] << 16) +
		((uint32_t)ptr[3] << 24);
	H_LOGI(TAG, "extended capabilities: 0x%"PRIx32,cap);
...
static void print_ext_capabilities(uint8_t * ptr)
{
	...
	if (cap & ESP_WLAN_UART_SUPPORT)
		H_LOGI(TAG, "\t * WLAN over UART");
	if (cap & ESP_BT_VHCI_UART_SUPPORT)
		H_LOGI(TAG, "\t * BT over UART (VHCI)");
#else
	H_LOGI(TAG, "\t No extended features. capabilities[%" PRIu32 "]", cap);
#endif
}
...
#define LENGTH_1_BYTE 1
	...
	*pos = low_thr_thesh;                              pos++;len++;
	...
	H_LOGI(TAG, "raw_tp_dir[%s], flow_ctrl: low[%u] high[%u]",
...
{
	H_LOGI(TAG, "transport_delayed_init");
	rpc_start();
	/* Add up cli */
#ifdef H_ESP_HOSTED_CLI_ENABLED
...
static int process_init_event(uint8_t *evt_buf, uint16_t len)
{
	...
	while (len_left) {
		...
		if (*pos == ESP_PRIV_CAPABILITY) {
...
#endif
			H_LOGI(TAG, "SDIO mode: slave: %s, host: %s",
			...
			H_LOGD(TAG, "Unsupported EVENT: %2x", *pos);
		}
		pos += (tag_len+2);
		len_left -= (tag_len+2);
	}
