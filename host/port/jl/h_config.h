/* host/port/jl/h_config.h
 * JL shadow of host/port/include/h_config.h.
 *
 * The canonical h_config.h lives in host/port/include/ and includes
 * "h_port_config.h". Because #include "" searches the directory containing
 * the including file first, that canonical header would always pick
 * host/port/include/h_port_config.h (the ESP-IDF fallback) instead of the
 * JL port config.
 *
 * By placing this shadow in host/port/jl/ (which is first in the include path),
 * every #include "h_config.h" resolves here, and the subsequent
 * #include "h_port_config.h" resolves to host/port/jl/h_port_config.h.
 */

#ifndef H_CONFIG_JL_H
#define H_CONFIG_JL_H
