...
#define SPI_HD_LOCK(x) do { \
    if (x) h_mutex_lock(spi_hd_bus_lock, H_BLOCK_MAX); \
} while (0);

#define SPI_HD_UNLOCK(x) do { \
    if (x) h_mutex_unlock(spi_hd_bus_lock); \
} while (0);
...
static h_mutex_t spi_hd_bus_lock = NULL;
...
void * h_spi_hd_bus_init(void)
{
    ...
    ctx = h_calloc(1, sizeof(spi_hd_ctx_t));
    assert(ctx);
    ...
    h_mutex_create(&spi_hd_bus_lock);
 ...
 * and error-checking. They are part of the current port contract
 * (accessed via g_h_transport.spi_hd_*) and will be revisited when
