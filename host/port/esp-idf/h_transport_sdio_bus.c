...
#define SDIO_LOCK(x) do { \
		if (x) h_mutex_lock(sdio_bus_lock, H_BLOCK_MAX); \
	} while (0);

#define SDIO_UNLOCK(x) do { \
		if (x) h_mutex_unlock(sdio_bus_lock); \
	} while (0);
...
static esp_err_t hosted_sdio_print_cis_information(sdmmc_card_t* card)
{
	...
	if (ret == ESP_ERR_INVALID_SIZE) {
		...
		uint8_t* temp_buf = h_malloc(temp_buf_size);
		assert(temp_buf);
...
void * h_sdio_bus_init(void)
{
		...
		if (ESP_TRANSPORT_OK == esp_hosted_transport_get_config(&pconfig)) {
			if (pconfig->transport_in_use == H_TRANSPORT_SDIO) {
				...
				if (ESP_TRANSPORT_OK == esp_hosted_sdio_get_config(&psdio_config)) {
					...
					h_memcpy(&context.config, psdio_config, sizeof(struct esp_hosted_sdio_config));
	...
	context.card = (sdmmc_card_t *)h_malloc(sizeof(sdmmc_card_t));
	if (!context.card) {
