...
# NOTE: 门槛 4 完成后, Linux mock 已覆盖全部 12 个 host/core/src 生产文件的
# 编译、链接与生产路径测试验证。脚本输出需与 9 号文档的三口径保持一致。
# 所有 host/core/src/ 文件 — 门槛 4 后全部纳入编译验证
ALL_CORE_SRCS="host/core/src/h_init.c host/core/src/h_rpc_core.c host/core/src/h_serial_if.c host/core/src/h_event.c host/core/src/h_api.c host/core/src/h_transport_util.c host/core/src/h_rpc_req.c host/core/src/h_rpc_rsp.c host/core/src/h_rpc_evt.c host/core/src/h_rpc_utils.c host/core/src/h_transport_drv.c host/core/src/h_rpc_wrap.c"
...
TEST_SRCS="tests/test_runner.c tests/test_osal.c tests/test_event.c tests/test_transport.c tests/test_rpc_core.c tests/test_rpc_bridge.c tests/test_wifi_type_conversion.c tests/unity/unity.c tests/stubs/rpc_wifi_stubs.c tests/stubs/protobuf_c_stubs.c tests/stubs/serial_stubs.c tests/stubs/transport_pserial_stubs.c tests/stubs/rpc_slaveif_stubs.c tests/stubs/transport_drv_stubs.c"
...
# ── 编译前打印两个口径下的集合 ──
# 注: WP 4.5 完成后 h_rpc_core.c 已移除 H_BUILD_TESTS, 全部 12 个文件均走生产路径。
echo "── mock 持续编译集合(host/core/src/ 总文件数 12)──"
MOCK_BUILD_CORE=(
...
echo "  编译覆盖率: ${#MOCK_BUILD_CORE[@]}/12 = $(awk "BEGIN { printf \"%.0f%%\", ${#MOCK_BUILD_CORE[@]} / 12 * 100 }")"
echo ""
...
echo "  覆盖率: ${#TESTABLE_CORE[@]}/12 = $(awk "BEGIN { printf \"%.0f%%\", ${#TESTABLE_CORE[@]} / 12 * 100 }")"
...
echo "  待办: ${#COMPILE_ONLY_CORE[@]}/12 (编译通过, 待链接)"
echo ""
...
echo "  已链接: ${#LINKED_BUT_NOT_TESTED[@]}/12 (编译+链接通过, 待生产路径测试覆盖)"
echo ""
...
if [ -n "$MISSING" ]; then
    ...
    echo "  ✓ MOCK_BUILD_CORE == ALL_CORE_SRCS (12/12)"
fi
...
echo "  ✓ All 12 core files compiled successfully"
echo ""
